(function($) {
	...
	//Tabs Box
	document.querySelectorAll('li.tab-btn').forEach(bt => {
			bt.onclick = e => {
		
				let btGroup = bt.closest('.tabs-box')
					btGroup.querySelectorAll('li.tab-btn').forEach(gBt =>
					gBt.classList.toggle('active-btn', gBt === bt))

				btGroup.querySelectorAll('div.tab').forEach(gDv =>
					gDv.classList.toggle('active-tab', gDv.id === bt.dataset.tab));
				}
	})
	// 
	if ($('.tabs-box').length) {
